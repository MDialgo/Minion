local tbl = {
	...
	--			function customfunction()
	--				local ilvlweapon = 1036719
	--				local animaweapon = 15238
	--				local entrancepos = {x = 130.99, y = -18, z = 463.33}
	--				local finalbosspos = {x = 156.6, y = -7.85, z = -351}
	--				if (math.distance2d(Player.pos, entrancepos) < 10 and not Player.incombat and not IsEquipped(ilvlweapon)) then
	--					EquipItem(ilvlweapon, 0)
	--				end
	--				if (math.distance2d(Player.pos, finalbosspos) < 10 and not Player.incombat and not IsEquipped(animaweapon)) then
	--					EquipItem(animaweapon, 0)
	--				end
	--			end
	--		]]
			--[[
			* Equip your Anima Weapon and put in console: d(Inventory:Get(1000):GetList()[1].id)
			  and replace the number after local animaweapon = ####
			* Equip your ilvl Weapon and if you have a HQ weapon, enter: d(Inventory:Get(1000):GetList()[1].hqid)
			  and replace the number after local ilvlweapon = ####,
				if yout ilvl weapon is NQ, use d(Inventory:Get(1000):GetList()[1].id)
			]]
	--	},
	},
	overheadmarkers = {},

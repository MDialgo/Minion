local tbl = {
	...
	prioritytarget = {
		[1] = {contentid = 3655, priority = 1, type = "phase 2 adds"},
	},
	ignoretarget = {3654},
	tankat = {
...
--[[
	...
	if HasBuff(Player.id, 284) and hasdebuff == 0 then
		KitanoiSettings.avoidingtime = Now()
		if GetRoleString(Player.job) == "Tank" then
			Player:MoveTo(tankx[i], tanky[i], tankz[i])
		else
			Player:MoveTo(otherx[i], othery[i], otherz[i])
		end
		i = i + 1
		hasdebuff = 1
	end
	if not HasBuff(Player.id, 284) and hasdebuff == 1 then
		hasdebuff = 0
		KitanoiSettings.avoidingtime = Now()
	end
	if i == table.getn(tankx) + 1 then
		i = 1
	end
--]]
}
